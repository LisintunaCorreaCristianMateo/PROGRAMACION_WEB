 ...
 */
 ( función( global, fábrica ) {
	...
	si ( tipo de módulo === "objeto" y tipo de módulo.exportaciones === "objeto" ) {
...
// es suficiente que todos esos intentos estén protegidos en un bloque try.
"uso estricto";
...
var esFuncion = funcion esFuncion( obj ) {
		...
		// Además, para el antiguo WebKit, typeof devuelve "función" para colecciones HTML
...
var documento = ventana.documento;
	...
	función DOMEval( código, nodo, doc ) {
		...
		script.texto = código;
		si ( nodo ) {
			para ( i en los atributos de script preservados ) {
...
función toType(obj) {
	...
	// Soporte: solo Android <=2.3 (RegExp funcional)
	devolver typeof obj === "objeto" || typeof obj === "función" ?
...
jQuery.fn = jQuery.prototype = {
	...
	pushStack: función( elementos ) {
		...
		// Construye un nuevo conjunto de elementos coincidentes con jQuery
...
jQuery.extend = jQuery.fn.extend = función() {
	...
	para ( ; i < longitud; i++ ) {
		...
		// Tratar únicamente con valores no nulos/indefinidos
		si ( ( opciones = argumentos[ i ] ) != null ) {
			...
			para (nombre en opciones) {
				...
				si ( profundo && copia && ( jQuery.isPlainObject( copia ) ||
					...
					objetivo[ nombre ] = jQuery.extend( deep, clone, copy );
...
jQuery.extend( {
	...
	isPlainObject: función(obj) {
		...
		// Utilice toString en lugar de jQuery.type para capturar objetos del host
		si ( !obj || toString.call( obj ) !== "[object Object]" ) {
			devuelve falso;
		}
	...
	// Evalúa un script en un contexto proporcionado; recurre al global
	// si no se especifica.
	...
	isXMLDoc: función( elem ) {
		var espacio de nombres = elem && elem.namespaceURI,
			docElem = elem && ( elem.ownerDocument || elem ).documentElement;

		// Suponga que hay HTML cuando documentElement aún no existe, como dentro
	...
	grep: función(elems, devolución de llamada, invertir) {
		...
		// Recorre el array, guardando solo los elementos
	...
	// jQuery.support no se usa en Core, pero otros proyectos lo adjuntan.
...
jQuery.each( "Booleano Número Cadena Función Matriz Fecha Expresión regular Objeto Error Símbolo".split( " " ),
	función(_i, nombre) {
...
función isArrayLike(obj) {
	...
	var length = !!obj && "longitud" en obj && obj.length,
		tipo = toType(obj);
...
// https://drafts.csswg.org/cssom/#modismos-de-serialización-comunes
var rcssescape = /([\0-\x1f\x7f]|^-?\d)|^-$|[^\x80-\uFFFF\w-]/g;

función fcssescape( ch, asCodePoint ) {
	si ( comoPuntoDeCódigo ) {
...
jQuery.escapeSelector = función(sel) {
	devolver (sel + "").reemplazar(rcssescape, fcssescape);
};
...
variedad yo,
	...
	booleanos = "marcado|seleccionado|asincrónico|enfoque automático|reproducción automática|controles|aplazar|deshabilitado|oculto|ismap|" +
	...
	pseudos = ":(" + identificador + ")(?:\\((" +
		...
		// Para reducir la cantidad de selectores que necesitan tokenizarse en el preFilter, prefiera los argumentos:
	...
	// Espacios en blanco iniciales y finales sin escape, capturando algunos caracteres que no son espacios en blanco que preceden a estos últimos
	...
	matchExpr = {
		...
		ETIQUETA: nueva expresión regular ( "^(" + identificador + "|[*])" ),
		...
		needsContext: nueva expresión regular ( "^" + espacio en blanco +
			"*[>+~]|:(par|impar|ecu|gt|lt|nésimo|primero|último)(?:\\(" + espacio +
	...
	// Escapes CSS
	// https://www.w3.org/TR/CSS21/syndata.html#caracteres-de-escapo
...
// Soporte: sólo IE <=9
// Acceder a document.activeElement puede generar un error inesperado
...
función find(selector, contexto, resultados, semilla) {
	var m, i, elem, nid, coincidencia, grupos, nuevoSelector,
		newContext = contexto && contexto.ownerDocument,

		// nodeType tiene como valor predeterminado 9, ya que el contexto tiene como valor predeterminado el documento
	...
	si ( !semilla ) {
		...
		si (documentoIsHTML) {
			...
			si ( !nonnativeSelectorCache[ selector + " " ] &&
				...
				// En tales casos, solucionamos el problema colocando un prefijo en cada selector.
...
/**
 * Soporte para pruebas usando un elemento
 * @param {Function} fn Pasa el elemento creado y devuelve un resultado booleano
 */
función assert( fn ) {
...
función createInputPseudo(tipo) {
	función de retorno (elem) {
		retorna nodeName( elem, "input" ) && elem.type === tipo;
	};
}
...
función createDisabledPseudo( deshabilitado ) {
	...
	// Falsos positivos conocidos: deshabilitado: fieldset[deshabilitado] > leyenda:nth-of-type(n+2) :can-disable
	función de retorno (elem) {
		...
		si ( "formulario" en elem ) {
			...
			si (elem.parentNode && elem.disabled === falso) {
				...
				// Los elementos de opción difieren de un grupo de opciones padre si está presente
				si ( "etiqueta" en elem ) {
		...
		// Los elementos restantes no están ni :habilitados ni :deshabilitados
		devuelve falso;
	};
}
 ...
 * @param {Elemento|Objeto} [nodo] Un elemento u objeto de documento que se usará para configurar el documento
 * @returns {Object} Devuelve el documento actual
 */
función setDocument( nodo ) {
	var subVentana,
		doc = nodo ? nodo.ownerDocument || nodo : preferredDoc;
	...
	// Los métodos getElementById rotos no recogen nombres establecidos programáticamente,
	// así que use una prueba indirecta getElementsByName
	soporte.getById = assert( función( el ) {
		documentElement.appendChild( el ).id = jQuery.expando;
		devolver !documento.getElementsByName ||
	...
	soporte.ámbito = afirmar( función() {
	...
	Expr.find.CLASS = función(nombreClase, contexto) {
		si ( tipo de contexto.getElementsByClassName !== "indefinido" && documentIsHTML ) {
			devolver contexto.getElementsByClassName( nombreClase );
		}
	};
	...
	// Construir expresiones regulares de QSA
	...
	afirmar(función(el) {
		...
		// Esto probablemente esté bien, pero por compatibilidad con versiones anteriores queremos mantener
		...
		si ( el.querySelectorAll( ":deshabilitado" ).length !== 2 ) {
	...
	sortOrder = función(a, b) {
		...
		si (comparar & 1 ||
			...
			// dos documentos; las comparaciones superficiales funcionan.
			// eslint-deshabilitar-siguiente-línea eqeqeq
			si ( a === documento || a.ownerDocument == preferredDoc &&
			...
			// dos documentos; las comparaciones superficiales funcionan.
			// eslint-deshabilitar-siguiente-línea eqeqeq
			si ( b === documento || b.ownerDocument == preferredDoc &&
	...
	si (documentIsHTML &&
		...
		intentar {
			...
			si ( ret || soporte.desconectadoMatch ||
					...
					// Asimismo, se dice que los nodos desconectados están en un documento.
					// fragmento en IE 9
					elem.documento && elem.documento.nodeType !== 11 ) {
				retorno ret;
			}
		} captura ( e ) {
...
find.contains = función( contexto, elemento ) {
	...
	devuelve jQuery.contains( contexto, elem );
};
...
jQuery.uniqueSort = función( resultados ) {
	...
	// Limpiar la entrada después de ordenar para liberar objetos
...
Expr = jQuery.expr = {
	...
	prefiltro: {
		ATTR: función(coincidencia) {
			...
			// Mueva el valor dado a match[3] ya sea entre comillas o sin ellas
		...
		PSEUDO: función(coincidencia) {
			...
			// Elimina los caracteres sobrantes de los argumentos no entre comillas
			} de lo contrario si ( sin comillas && rpseudo.test( sin comillas ) &&

				// Obtener el excedente de tokenizar (recursivamente)
	...
	filtro: {
		...
		ATTR: función(nombre, operador, comprobar) {
			función de retorno (elem) {
				var resultado = buscar.attr( elem, nombre );
			...
			devolver primero === 1 y último === 0 ?
				...
				función( elem, _contexto, xml ) {
					...
					si ( padre ) {
						...
						si ( reenviar && usarCache ) {

							// Busca `elem` desde un índice previamente almacenado en caché
							externalCache = padre[expandir] || ( padre[expandir] = {});
							...
							mientras ( ( nodo = ++nodeIndex && nodo && nodo[ dir ] ||
								...
								si ( nodo.tipoNodo === 1 && ++diff && nodo === elem ) {
							...
							// xml :nth-child(...)
							// o :nth-último-hijo(...) o :nth(-último)?-de-tipo(...)
							si ( diff === falso ) {
		...
		PSEUDO: función( pseudo, argumento ) {
			...
			argumentos variables,
				fn = Expr.pseudos[ pseudo ] || Expr.setFilters[ pseudo.toLowerCase() ] ||
					find.error( "pseudo no compatible: " + pseudo );
	...
	pseudos: {
		...
		no: markFunction( función( selector ) {
			...
			var entrada = [],
				resultados = [],
				matcher = compilar( selector.reemplazar( rtrimCSS, "$1" ) );

			¿devolver matcher[expando]?
				markFunction( función( semilla, coincidencias, _contexto, xml ) {
					elemento var,
						no coincidente = comparador(semilla, nulo, xml, []),
		...
		tiene: markFunction(función(selector) {
			función de retorno (elem) {
				devolver find(selector, elem).length > 0;
			};
		} ),
		...
		idioma: markFunction( función( idioma ) {
			...
			// El valor de lang debe ser un identificador válido
		...
		foco: función( elem ) {
			devolver elem === safeActiveElement() &&
				documento.hasFocus() &&
		...
		vacío: función( elem ) {
			...
			// :empty es negado por el elemento (1) o nodos de contenido (texto: 3; cdata: 4; entidad ref: 5),
		...
		entrada: función( elem ) {
			devolver rinputs.test( elem.nodeName );
		},

		botón: función( elem ) {
			devuelve nodeName( elem, "entrada") y elem.type === "botón" ||
				nodeName( elem, "botón" );
		},
		...
		par: createPositionalPseudo(función(matchIndexes, longitud) {
			var i = 0;
			para ( ; i < longitud; i += 2 ) {
				matchIndexes.push( i );
			}
...
función tokenize( selector, parseOnly ) {
	...
	mientras (hasta ahora) {
		...
		si ( !coincidido || ( coincidencia = rcomma.exec( soFar ) ) ) {
			si (coincide) {
		...
		si ( ( coincidencia = rleadingCombinator.exec( soFar ) ) ) {
			...
			tokens.push( {
				...
				// Transfiere combinadores descendientes al espacio
		...
		para ( escriba en Expr.filter ) {
			si ( ( coincidencia = matchExpr[ tipo ].exec( soFar ) ) && ( !preFilters[ tipo ] ||
...
función addCombinator(combinador, comparador, base) {
	...
	devolver combinator.first ?

		// Verificar con el ancestro/elemento precedente más cercano
		función( elem, contexto, xml ) {
			mientras ( ( elem = elem[ dir ] ) ) {
				si ( elem.nodeType === 1 || comprobarNonElements ) {
					devuelve matcher( elem, contexto, xml );
				}
			}
			devuelve falso;
		} :
				...
				mientras ( ( elem = elem[ dir ] ) ) {
					si ( elem.nodeType === 1 || comprobarNonElements ) {
						...
						si ( saltar && nodeName( elem, saltar ) ) {
							...
							// Reutilizar newcache para que los resultados se propaguen a los elementos anteriores
							...
							si ( ( newCache[ 2 ] = matcher( elem, context, xml ) ) ) {
								devuelve verdadero;
							}
						}
					}
...
función setMatcher(preFilter, selector, matcher, postFilter, postFinder, postSelector) {
	...
	return markFunction( función( semilla, resultados, contexto, xml ) {
		var temp, i, elem, matcherOut,
			...
			// Prefiltro para obtener la entrada del comparador, preservando un mapa para la sincronización de los resultados de las semillas
		...
		si ( postFilter ) {
			...
			// Desempareja los elementos fallidos moviéndolos nuevamente a matcherIn
			i = temp.longitud;
			mientras ( i-- ) {
				si ( ( elem = temp[ i ] ) ) {
...
función matcherFromTokens( tokens ) {
	var checkContext, matcher, j,
		...
		implícitoRelativo = principalRelativo || Expr.relativo[ " " ],
	...
	var bySet = setMatchers.length > 0,
		...
		superMatcher = función(semilla, contexto, xml, resultados, más externo) {
			var elem, j, comparador,
				...
				conjuntoMatched = [],
				contextBackup = contextomásexterno,
			...
			si (más externo) {
				...
				// dos documentos; las comparaciones superficiales funcionan.
				// eslint-deshabilitar-siguiente-línea eqeqeq
				contextoExterior = contexto == documento || contexto || más externo;
			}

			// Agregue elementos pasando elementMatchers directamente a los resultados
			// Soporte: solo iOS <=7 - 9
			...
			para ( ; i !== len && ( elem = elems[ i ] ) != null; i++ ) {
				...
				// Rastrear elementos no coincidentes para filtros establecidos
				si ( porSet ) {
			...
			// no hay comparadores de elementos y ninguna semilla.
			...
			if (porSet && i! == matchedCount) {
				j = 0;
				mientras ( ( matcher = setMatchers[ j++ ] ) ) {
				...
				si ( semilla ) {
					...
					si ( matchedCount > 0 ) {
						mientras ( i-- ) {
							si ( !( no coincidente[ i ] || conjuntoCoincidencia[ i ] ) ) {
				...
				// Las coincidencias de conjuntos sin semillas que suceden a múltiples coincidencias exitosas estipulan la clasificación
			...
			// Anular la manipulación de variables globales mediante comparadores anidados
			si (más externo) {
				dirruns = dirrunsUnique;
...
función compilar( selector, match /* Solo para uso interno */ ) {
	...
	si ( !en caché ) {
		...
		en caché = compilerCache( selector,
			matcherFromGroupMatchers(elementoMatchers, conjuntoMatchers) );
 ...
 * Función selectora construida con jQuery selector compile
 * @param {Elemento} contexto
 * @param {Array} [resultados]
 * @param {Array} [semilla] Un conjunto de elementos con los que comparar
 */
función select(selector, contexto, resultados, semilla) {
	...
	// Intente minimizar las operaciones si solo hay un selector en la lista y ninguna semilla
	...
	si ( coincidencia.longitud === 1 ) {
		...
		mientras ( i-- ) {
			...
			// Abortar si conseguimos un combinador
			si ( Expr.relativo[ ( tipo = token.tipo ) ] ) {
				romper;
			}
			si ( ( encontrar = Expr. encontrar[ tipo ] ) ) {

				// Búsqueda, ampliando el contexto para los principales combinadores hermanos
...
// Soporte: Android <=4.0 - 4.1+
// Los nodos separados se suceden *unos a otros* de forma confusa
soporte.sortDetached = assert( función( el ) {

	// Debería devolver 1, pero devuelve 4 (siguiente)
	devuelve el.compareDocumentPosition(document.createElement("fieldset" ) ) & 1;
} );
...
var rootjQuery,
	...
	// Atajo simple #id para mayor velocidad
	rquickExpr = /^(?:\s*(<[\w\W]+>)[^>]*|#([\w-]+))$/,
	...
	init = jQuery.fn.init = función(selector, contexto, raíz) {
		...
		si ( tipo de selector === "cadena" ) {
			...
			// Coincide con el HTML o asegúrese de que no se especifique ningún contexto para #id
				...
				si ( coincide[ 1 ] ) {
					...
					si ( rsingleTag.test( match[ 1 ] ) && jQuery.isPlainObject( contexto ) ) {
						para (coincidir en contexto) {
							...
							si ( esFuncion( esto[ coincidencia ] ) ) {
...
jQuery.fn.extend( {
	tiene: función(objetivo) {
		...
		devuelve este.filtro(función() {
			var i = 0;
			para ( ; i < l; i++ ) {
				si ( jQuery.contains( this, targets[ i ] ) ) {
					devuelve verdadero;
				}
			}
		} );
	...
	más cercano: función(selectores, contexto) {
		var cur,
			...
			l = esta.longitud,
			coincidido = [],
			objetivos = tipo de selectores !== "string" && jQuery( selectores );
		...
		si ( !rneedsContext.test(selectores) ) {
			para ( ; i < l; i++ ) {
				for ( cur = this[ i ]; cur && cur !== contexto; cur = cur.parentNode ) {
					...
					si ( cur.nodeType < 11 y ( objetivos ?
						objetivos.índice(cur) > -1:

						// No pase elementos no esenciales a jQuery#find
						cur.nodeType === 1 &&
	...
	índice: función( elem ) {
		...
		si ( tipo de elemento === "cadena") {
			devolver indexOf.call( jQuery( elem ), this[ 0 ] );
		}

		// Localiza la posición del elemento deseado
		devuelve indexOf.call(este,
...
jQuery.each( {
	...
	nextAll: función( elem ) {
		devolver dir( elem, "nextSibling" );
	},
	prevAll: función( elem ) {
		devolver dir( elem, "hermanoanterior" );
	},
	nextUntil: función( elem, _i, hasta ) {
		devolver dir( elem, "nextSibling", hasta );
	},
	prevHasta: función( elem, _i, hasta ) {
		devolver dir( elem, "hermanoanterior", hasta );
	},
	...
	jQuery.fn[ nombre ] = función( hasta, selector ) {
		...
		devuelve este.pushStack(coincidido);
	};
} );
...
// Convierte opciones con formato de cadena en opciones con formato de objeto
 ...
 * después de que la lista haya sido despedida inmediatamente con el último "memorizado"
...
jQuery.Callbacks = función(opciones) {
	...
	var // Marca para saber si la lista se está activando actualmente
		...
		// Bandera para saber si la lista ya fue activada
		despedido,
		...
		fuego = función() {
			...
			// respetando las anulaciones de firingIndex y los cambios en tiempo de ejecución
			disparó = disparando = verdadero;
			para ( ; cola.longitud; índiceDeDisparo = -1 ) {
				...
				mientras ( ++firingIndex < lista.length ) {
					...
					si ( lista[ índiceDeDisparo ]. aplicar( memoria[ 0 ], memoria[ 1 ] ) === falso &&
						...
						índiceDeDisparo = lista.longitud;
						memoria=falso;
					}
				}
			}
		...
		// Objeto de devoluciones de llamadas reales
		yo mismo = {
			...
			añadir: función() {
				si ( lista ) {
					...
					si ( memoria && !disparo ) {
						índiceDeDisparo = lista.longitud - 1;
						cola.push( memoria );
					}
					...
					( función agregar( args ) {
						jQuery.each(args, function(_, arg) {
							si ( esFuncion( arg ) ) {
								si ( !opciones.únicas || !self.tiene( arg ) ) {
									lista.push( arg );
								}
			...
			// Abortar cualquier ejecución actual o pendiente
			...
			// Deshabilitar .fire
			// También deshabilite .add a menos que tengamos memoria (ya que no tendría ningún efecto)
			...
			// Llamar a todas las devoluciones de llamada con el contexto y argumentos dados
...
función adoptarValor(valor, resolver, rechazar, sinValor) {
	...
	intentar {
		...
		// Otros thenables
		} de lo contrario si ( valor && esFuncion( ( método = valor.then ) ) ) {
			método.call( valor, resolver, rechazar );

		// Otros no-entornos
		} demás {

			// Controla los argumentos de `resolver` permitiendo que Array#slice convierta el booleano `noValue` a un entero:
			// * falso: [ valor ].slice( 0 ) => resolve( valor )
			// * verdadero: [ valor ].slice( 1 ) => resolve()
			resolve.apply( indefinido, [ valor ].slice( sinValor ) );
		}
	...
	// Diferido#then para suprimir condicionalmente el rechazo.
	} catch ( valor ) {
...
jQuery.extend( {
	...
	Diferido: función(func) {
		var tuplas = [
				...
				// ... .luego controladores, índice de argumento, [estado final]
			...
			promesa = {
				...
				tubería: función( /* fnHecho, fnFallo, fnProgreso */ ) {
					var fns = argumentos;

					devolver jQuery.Deferred( función( newDefer ) {
						jQuery.each( tuplas, función( _i, tupla ) {
							...
							diferido[ tupla[ 1 ] ]( función() {
								...
								si ( devuelto && esFuncion( devuelto.promesa ) ) {
									devuelto.promesa()
										.progress( nuevoAplazar.notificar )
										.done( nuevoAplazamiento.resolver )
										.fail( nuevoAplazamiento.rechazar );
								} demás {
						...
						función de retorno() {
							var que = esto,
								...
								podríaLanzar = función() {
									...
									// https://promesaplus.com/#punto-59
									// Ignorar intentos de doble resolución
									si ( profundidad < profundidad máxima ) {
										devolver;
									}

									devuelto = manejador.apply( eso, args );

									// Soporte: Sección Promesas/A+ 2.3.1
									// https://promesaplus.com/#punto-48
									si ( devuelto === diferido.promesa() ) {
										lanzar nuevo TypeError( "Entonces permite la auto-resolución" );
									}

									// Soporte: Promesas/A+ secciones 2.3.3.1, 3.5
									// https://promesaplus.com/#punto-54
									// https://promesaplus.com/#punto-75
									// Recuperar `then` sólo una vez
									entonces = devolvió &&

										// Soporte: Promesas/A+ sección 2.3.4
										// https://promesaplus.com/#punto-64
										// Solo comprobar la capacidad de los objetos y funciones
								...
								// Sólo los procesadores normales (resuelven) capturan y rechazan excepciones
								proceso=especial ?
									podríaLanzar:
									función() {
										intentar {
											...
											// https://promesaplus.com/#punto-61
											// Ignorar excepciones posteriores a la resolución
											si ( profundidad + 1 >= profundidad máxima ) {
							...
							// https://promesaplus.com/#punto-57
							// Re-resuelva las promesas inmediatamente para evitar el rechazo falso de
							// errores posteriores
							si ( profundidad ) {
								proceso();
							} demás {
								...
								// El alias obsoleto del anterior. Aunque el nombre lo sugiere
		...
		jQuery.each( tuplas, función( i, tupla ) {
			...
			si ( cadena de estado ) {
				lista.add(
					...
					// manejadores_rechazados.deshabilitar
					// controladores_cumplidos.deshabilitar
					tuplas[3 - i][3].deshabilitar,

					// bloqueo de devoluciones de llamada de progreso
					tuplas[ 0 ][ 2 ].lock,
			...
			// manejadores_rechazados.fire
			lista.add( tupla[ 3 ].fire );

			// diferido.notificar = función() { diferido.notificarCon(...) }
			// diferido.resolver = función() { diferido.resolverCon(...) }
			...
			// diferido.rechazarCon = lista.dispararCon
			diferido[ tupla[ 0 ] + "Con" ] = lista.fireWith;
		} );

		// Haz de lo aplazado una promesa
	...
	cuando: función( valor_único ) {
		variedad
			...
			updateFunc = función( i ) {
				función de retorno (valor) {
					...
					si ( !(--restante) ) {
						primaria.resolveWith( resolverContextos, resolverValores );
					}
				};
			};
		...
		si ( restante <= 1 ) {
			adoptarValor(valorSingular, principal.hecho(funciónActualizar(i)).resolver, principal.rechazar,
				!restante );

			// Utilice .then() para desenvolver thenables secundarios (cf. gh-3000)
			si (primario.estado() === "pendiente" ||
				esFuncion( resolverValores[ i ] && resolverValores[ i ].then ) ) {

				devolver primario.then();
			}
		}
...
jQuery.Deferred.exceptionHook = función(error, asyncError) {
	...
	si ( ventana.consola && ventana.consola.advertencia && error && rerrorNames.test( error.nombre ) ) {
		window.console.warn( "jQuery.Deferred excepción: " + error.message,
...
jQuery.extend( {
	...
	¿Está el DOM listo para usarse? Establézcalo como verdadero cuando ocurra.
	isReady: falso,

	// Un contador para rastrear cuántos elementos hay que esperar antes
	Se activa el evento listo. Véase trac-6781
	listoEspera: 1,
	...
	listo: función(esperar) {
		...
		// Si se activa un evento DOM Ready normal, decrementar y esperar si es necesario
		si ( esperar !== verdadero && --jQuery.readyWait > 0 ) {
			devolver;
		}
...
si (document.readyState === "completo" ||
	...
	// Manéjelo de forma asincrónica para permitir que los scripts tengan la oportunidad de retrasar su preparación
...
var acceso = función( elems, fn, clave, valor, encadenable, emptyGet, raw ) {
	...
	si ( toType( clave ) === "objeto" ) {
		...
		para ( i en clave ) {
			acceso( elems, fn, i, tecla[ i ], verdadero, emptyGet, raw );
		}
...
// Convierte guiones a camelCase; utilizado por los módulos CSS y de datos
// Compatibilidad: IE <=9 - 11, Edge 12 - 15
// Microsoft olvidó añadir su prefijo de proveedor (trac-9572)
...
var acceptData = función( propietario ) {
	...
	// - Objeto
	// - Cualquier
	devolver propietario.tipoNodo === 1 || propietario.tipoNodo === 9 || !( +propietario.tipoNodo );
};
...
Datos.prototipo = {
	...
	caché: función(propietario) {
		...
		si ( !valor ) {
			...
			// Podemos aceptar datos para nodos que no sean elementos en navegadores modernos,
			// pero no deberíamos, ver trac-8335.
			// Siempre devuelve un objeto vacío.
			si ( acceptData( propietario ) ) {

				// Si es un nodo, es poco probable que se convierta en una cadena o se repita en un bucle.
...
//
// 1. Hacer cumplir la superficie de la API y la compatibilidad semántica con la rama 1.9.x
...
// 6. Proporcionar una ruta clara para la actualización de la implementación de WeakMap en 2014
...
jQuery.extend( {
	...
	removeData: función(elem, nombre) {
	...
	datos: función( clave, valor ) {
		...
		si ( tipo de clave === "objeto" ) {
			devuelve esto.each( función() {
				dataUser.set(este, clave);
			} );
		}

		devolver acceso(este, función(valor) {
			var datos;
			...
			si ( elem && valor === indefinido ) {
				...
				// La clave siempre estará en formato camelCased en los datos
				datos = datosUsuario.get( elem, clave );
				si (datos !== indefinido) {
					devolver datos;
				}
...
jQuery.extend( {
	cola: función(elem, tipo, datos) {
		...
		si ( elem ) {
			...
			// Acelere la salida de la cola saliendo rápidamente si se trata solo de una búsqueda
			si (datos) {
	...
	desencolar: función(elem, tipo) {
		...
		si ( fn ) {
			...
			// Agregue un centinela de progreso para evitar que la cola de efectos se bloquee.
			// desencolado automáticamente
			si ( tipo === "fx" ) {
				cola.unshift( "en progreso" );
			}

			// Limpiar la última función de parada de cola
	...
	_queueHooks: función( elem, tipo ) {
		var clave = tipo + "queueHooks";
		devolver datosPriv.get( elem, clave ) || datosPriv.access( elem, clave, {
...
jQuery.fn.extend( {
	...
	// Obtener una promesa resuelta cuando las colas de un determinado tipo
...
var documentElement = document.documentElement;
	...
	// Esto genera errores. Necesitamos verificar `getRootNode`.
...
función ajustarCSS( elem, prop, valueParts, tween ) {
	var ajustado, escala,
		maxIteraciones = 20,
	...
	si ( initialInUnit && initialInUnit[ 3 ] !== unidad ) {
		...
		mientras (maxIteraciones--) {
			...
			// Evaluamos y actualizamos nuestra mejor estimación (duplicando las estimaciones que resultan en cero).
			// Finaliza si la escala es igual o cruza 1 (lo que hace que el producto antiguo*nuevo no sea positivo).
...
jQuery.fn.extend( {
	mostrar: función() {
		devuelve showHide(esto, verdadero);
	},
	ocultar: función() {
		devolver showHide(este);
	},
...
( función() {
	...
	// Soporte: solo Android 4.0 - 4.3
	// Verificar el estado perdido si el nombre está configurado (trac-11217)
	...
	// Soporte: solo Android <=4.1
	// Las versiones antiguas de WebKit no clonan correctamente el estado marcado en los fragmentos
...
var wrapMap = {
	...
	// De la misma forma que lo hacen los analizadores de sopa de etiquetas. Por lo tanto, no podemos acortar
...
// Marcar los scripts como si ya hubieran sido evaluados
	...
	para ( ; i < l; i++ ) {
		...
		si ( elem || elem === 0 ) {
			...
			// Convertir un nodo que no sea HTML en un nodo de texto
				...
				nodos.push( context.createTextNode( elem ) );
			...
			// Convertir HTML en nodos DOM
				...
				// Desciende a través de envoltorios hasta el contenido correcto
				j = wrap[ 0 ];
				mientras ( j-- ) {
					tmp = tmp.lastChild;
				}
...
función on( elem, tipos, selector, datos, fn, uno ) {
	var origFn, tipo;
...
jQuery.evento = {
	...
	agregar: función( elem, tipos, manejador, datos, selector ) {
		...
		mientras ( t-- ) {
			...
			// Si el evento cambia su tipo, utilice los controladores de eventos especiales para el tipo cambiado
			especial = jQuery.event.special[ tipo ] || {};

			// Si se define el selector, determina el tipo de API de evento especial, de lo contrario, el tipo dado
			tipo = ( selector ? especial.tipoDelegado : especial.tipoBind ) || tipo;

			// Actualización especial basada en el nuevo tipo de restablecimiento
			especial = jQuery.event.special[ tipo ] || {};
	...
	eliminar: función( elem, tipos, manejador, selector, mappedTypes ) {
		...
		mientras ( t-- ) {
			...
			mientras ( j-- ) {
				...
				si ( ( tiposMappedTypes || origType === handleObj.origType ) &&
			...
			// (evita la posibilidad de una recursión interminable durante la eliminación de controladores de eventos especiales)
			si ( origCount && !handlers.length ) {
				si ( !special.teardown ||
					special.teardown.call( elem, espacios de nombres, elemData.handle ) === falso ) {
		...
		//Eliminar datos y el expando si ya no se utiliza
		si ( jQuery.isEmptyObject( eventos ) ) {
			dataPriv.remove( elem, "manejar eventos" );
		}
	},

	despacho: función(eventoNativo) {
		...
		var i, j, ret, emparejado, handleObj, handlerQueue,
			...
			evento = jQuery.event.fix( eventoNativo ),
		...
		mientras ( ( coincidente = handlerQueue[ i++ ] ) && !event.isPropagationStopped() ) {
			evento.currentTarget = matched.elem;
			...
			mientras ( ( handleObj = matched.handlers[ j++ ] ) &&
				...
				si ( !evento.rnamespace || handleObj.namespace === falso ||
	...
	manejadores: función(evento, manejadores) {
		var i, handleObj, sel, manejadorescoincidentes, selectorescoincidentes,
			handlerQueue = [],
		...
		si ( delegateCount &&
			...
			// Compatibilidad: solo IE 11
			// ...pero no los "clics" de las teclas de flecha de las entradas de radio, que pueden tener `botón` -1 (gh-2343)
			!( tipo.evento === "clic" && botón.evento >= 1 ) ) {

			para ( ; cur !== esto; cur = cur.parentNode || esto ) {

				// No comprobar elementos no esenciales (trac-13208)
			...
			obtener: isFunction( gancho ) ?
				función() {
					si ( este.eventooriginal ) {
						retorno hook(this.originalEvent);
					}
				} :
				función() {
					si ( este.eventooriginal ) {
	...
	corrección: función(eventoOriginal) {
		¿devolver originalEvent[jQuery.expando]?
			Evento original:
			nuevo jQuery.Event(eventoEventooriginal);
	},

	especial: {
		carga: {

			// Evita que los eventos image.load activados se transmitan a window.load
			noBubble: verdadero
		},
		haga clic: {
			...
			configuración: función(datos) {
				...
				// Para una compresión mutua con _default, reemplace el acceso `this` con una variable local.
				...
				si ( rcheckableType.test( el.type ) &&
					...
					leverageNative( el, "clic", verdadero );
				}
			...
			disparador: función(datos) {
				...
				// Devuelve un valor distinto de falso para permitir la propagación normal de la ruta del evento
				devuelve verdadero;
			},
...
función leverageNative( el, tipo, isSetup ) {
	...
	jQuery.event.add( el, tipo, {
		...
		manejador: función(evento) {
			...
			// Dispara un evento sintético interno con los argumentos originales
				...
				// Se ejecutan controladores nativos en el mismo elemento. En el objetivo, esto se logra.
				// deteniendo la propagación inmediata solo en el evento jQuery. Sin embargo,
				// el evento nativo es reenvuelto por uno jQuery en cada nivel del
				...
				// enfoque/desenfoque que no crea burbujas, pero también detiene los clics en las casillas de verificación
...
jQuery.removeEvent = función(elem, tipo, identificador) {
...
jQuery.Event = función( src, props ) {
...
// https://www.w3.org/TR/2003/WD-DOM-Nivel-3-Eventos-20030331/ecma-script-binding.html
jQuery.Event.prototype = {
	constructor: jQuery.Event,
	isDefaultPrevented: devuelveFalse,
	isPropagationStopped: returnFalse,
	isImmediatePropagationStopped: devuelveFalse,
	isSimulated: falso,
...
jQuery.each( { foco: "enfocar", desenfoque: "enfocar" }, función( tipo, delegateType ) {

	función focusMappedHandler( evento nativo ) {
		si ( documento.documentMode ) {

			// Compatibilidad: IE 11+
			// Adjunte un único controlador de enfoque/enfoque en el documento mientras alguien lo desea
			// enfoque/desenfoque. Esto se debe a que los primeros son sincronizados en IE, mientras que los segundos
			// son asincrónicos. En otros navegadores, todos esos controladores se invocan sincrónicamente.
			...
			var identificador = dataPriv.get (esto, "identificador"),
				evento = jQuery.event.fix( eventoNativo );
			event.type = nativeEvent.type === "focusin" ? "focus" : "desenfoque";
			evento.isSimulated = verdadero;

			// Primero, maneja el enfoque de entrada y el de salida
			manejar(eventoNativo);

			// ...luego, maneja el enfoque/desenfoque
			//
			// el enfoque/desenfoque no burbujea mientras que el enfoque/desenfoque sí lo hace; simule el primero solo
			// invocando el controlador en el nivel inferior.
			si ( evento.objetivo === evento.objetivoactual ) {

				// La parte de configuración llama a `leverageNative`, que, a su vez, llama a
				// `jQuery.event.add`, por lo que el controlador del evento ya estará configurado
				//en este punto.
				manejar( evento );
			}
		} demás {
			...
			// mientras alguien quiere enfocarse/enfocarse.
	...
	jQuery.event.special[tipo] = {
		...
		configuración: función() {
			...
			// Reclama el primer manejador
			// dataPriv.set(este, "enfoque", ... )
			// dataPriv.set(this, "desenfoque", ...)
			leverageNative(este, tipo, verdadero);
			...
			si ( documento.documentMode ) {
				...
				si ( !adjunta ) {
					este.addEventListener(delegadoType, focusMappedHandler);
				}
		...
		desmontaje: función() {
			...
			si ( documento.documentMode ) {
				...
				si ( !adjunta ) {
					este.removeEventListener(delegadoType, focusMappedHandler);
	...
	jQuery.event.special[tipoDelegado] = {
		configuración: función() {
			...
			dataPriv.set (soporte de datos, tipo de delegado, (adjunta || 0) + 1);
		},
		desmontaje: función() {
			var doc = este.ownerDocument || este.documento || este,
				dataHolder = documento.documentMode ? esto: doc,
...
jQuery.each( {
	...
	pointerleave: "puntero de salida"
}, función( orig, fix ) {
	jQuery.event.special[ orig ] = {
		delegateType: corrección,
		bindType: arreglar,
...
jQuery.fn.extend( {
	...
	uno: función( tipos, selector, datos, fn ) {
		retorna on( esto, tipos, selector, datos, fn, 1 );
	},
...
función fixInput( src, dest ) {
	...
	// No se puede conservar el estado marcado de una casilla de verificación o un botón de opción clonado.
...
función domManip(colección, argumentos, devolución de llamada, ignorado) {
	...
	var fragmento, primero, scripts, tieneScripts, nodo, doc,
		yo = 0,
	...
	si (valorEsFuncion ||
		...
		devolver colección.each( función( índice ) {
	...
	si ( l ) {
		...
		si ( primero || ignorado ) {
			...
			// se vacia incorrectamente en determinadas situaciones (trac-8070).
			para ( ; i < l; i++ ) {
				nodo = fragmento;
				...
				para ( i = 0; i < hasScripts; i++ ) {
					...
					si ( rscriptType.test( nodo.type || "" ) &&
						...
						si ( nodo.src && ( nodo.tipo || "" ).toLowerCase() !== "módulo" ) {
							...
							// Desempaquetar una sección CDATA que contiene el contenido del script. Esto no debería ser...
							// necesario ya que en los documentos XML ya no son visibles cuando
							// inspeccionar el contenido de los elementos y en los documentos HTML no tienen
							// significado, pero conservamos esa lógica para compatibilidad con versiones anteriores.
							Esto se eliminará por completo en la versión 4.0. Véase gh-4904.
...
jQuery.extend( {
	...
	clon: función( elem, datosYEventos, deepDataAndEvents ) {
		...
		si ( datosYEventos ) {
			si ( deepDataAndEvents ) {
				...
				para ( i = 0, l = srcElements.length; i < l; i++ ) {
					cloneCopyEvent(elementos_de_origen[i],elementos_de_destino[i]);
				}
			} demás {
...
jQuery.fn.extend( {
		...
		devolver acceso(este, función(valor) {
			valor de retorno === ¿indefinido?
				...
				esto.vacío().cada( función() {
					si ( este.tipoDeNodo === 1 || este.tipoDeNodo === 11 || este.tipoDeNodo === 9 ) {
	...
	anteponer: función() {
		devuelve domManip(this, argumentos, función(elem) {
			si ( este.tipoDeNodo === 1 || este.tipoDeNodo === 11 || este.tipoDeNodo === 9 ) {
				var objetivo = manipulaciónObjetivo(este, elem);
				objetivo.insertarAntes( elem, objetivo.primerHijo );
			}
		} );
	},
	...
	clon: función(datosYEventos, deepDataAndEventos) {
		...
		devuelve este.map( función() {
			devuelve jQuery.clone(this, datosYEventos, deepDataAndEvents);
		} );
	},
...
jQuery.each( {
	...
	jQuery.fn[ nombre ] = función( selector ) {
		...
		para ( ; i <= último; i++ ) {
			elems = i === último ? este : this.clone( verdadero );
...
var rboxStyle = nueva expresión regular (cssExpand.join ("|" ), "i" );
...
( función() {
	...
	función computeStyleTests() {
		...
		div.style.cssTexto =
			"posición:relativa;mostrar:bloque;tamaño del cuadro:cuadro del borde;desbordamiento:desplazamiento;" +
		...
		// Soporte: solo Android 4.0 - 4.3, Firefox <=3 - 44
	...
	// Finalizar anticipadamente en entornos limitados (sin navegador)
	si ( !div.style ) {
		devolver;
	}
	...
	jQuery.extend( soporte, {
		...
		// establecido en CSS mientras las propiedades `offset*` informen valores correctos.
		...
		reliableTrDimensions: función() {
			...
			si (reliableTrDimensionsVal == null) {
				...
				tr.style.cssText = "tamaño del cuadro: cuadro de contenido; borde: 1px sólido";
...
función curCSS( elem, nombre, calculado ) {
	...
	calculado = calculado || obtenerEstilos( elem );
	...
	si (calculado) {
		...
		ret = computed.getPropertyValue( nombre ) || computed[ nombre ];
		...
		si ( isCustomProp && ret ) {
			...
			Firefox solo recorta los espacios iniciales. Chrome simplemente los contrae.
			// espacios iniciales y finales en un solo espacio.
			//
			// Regrese a "indefinido" si se devuelve una cadena vacía.
			...
			// permitiéndonos diferenciarlos sin una penalización en el rendimiento
			// y devolver "undefined" se alinea con el jQuery anterior.
			//
			...
			// (que *son* espacios en blanco CSS)
			// https://www.w3.org/TR/css-syntax-3/#preprocesamiento-de-entrada
		...
		// pero el ancho parece ser confiablemente en píxeles.
		// Esto va en contra del borrador de especificación de CSSOM:
...
función addGetHookIf( funciónCondición, funciónGancho ) {
	...
	devolver {
		obtener: función() {
			si ( condiciónFn() ) {
				...
				// (dependencia faltante), elimínela.
				eliminar esto.get;
				devolver;
			}

			//Se necesita un gancho; redefínalo para que la prueba de soporte no se ejecute nuevamente.
...
// Devuelve una propiedad con prefijo jQuery.cssProps o del proveedor potencialmente asignada
función finalPropName (nombre) {
	...
	devolver sellerProps[ nombre ] = sellerPropName( nombre ) || nombre;
}
...
función setPositiveNumber(_elem, valor, restar) {
	...
	¿partidos de vuelta?
		...
		// Protéjase contra "restar" no definido, por ejemplo, cuando se usa como en cssHooks
		Math.max( 0, coincide[ 2 ] - ( resta || 0 ) ) + ( coincide[ 3 ] || "px" ) :
		valor;
}

función boxModelAdjustment( elem, dimensión, caja, isBorderBox, estilos, computedVal ) {
	...
	para ( ; i < 4; i += 2 ) {
		...
		// "relleno" o "margen"
			...
			// Para "contenido" o "relleno", restar el borde
			si ( caja !== "margen" ) {
				delta -= jQuery.css( elem, "borde" + cssExpand[ i ] + "Ancho", verdadero, estilos );
			}
		}
	...
	// Tenga en cuenta el margen de desplazamiento positivo del cuadro de contenido cuando se solicite proporcionando computedVal
	si ( !isBorderBox && valorComputado >= 0 ) {

		// offsetWidth/offsetHeight es una suma redondeada del contenido, el relleno, el margen de desplazamiento y el borde
...
función getWidthOrHeight( elem, dimensión, extra ) {
	...
	var estilos = getStyles (elem),
		...
		// Falsifique el contenido del cuadro hasta que sepamos que es necesario para saber el valor verdadero.
	...
	// Soporte: Firefox <=54
	// Devuelve un valor que no sea un píxel y que pueda confundirte, o finge ignorancia, según corresponda.
	...
	// En esos casos, se puede confiar en que el valor calculado sea el borde del cuadro.
...
jQuery.extend( {
	...
	estilo: función( elem, nombre, valor, extra ) {
		...
		si ( valor !== indefinido ) {
			...
			// Las propiedades background-* afectan los valores del clon original
			si ( !support.clearCloneStyle && valor === "" && nombre.indexOf( "fondo" ) === 0 ) {
				estilo[ nombre ] = "heredarse";
			}
	...
	css: función(elem, nombre, extra, estilos) {
		...
		// Hacer numérico si se forzó o se proporcionó un calificador y val parece numérico
		si ( extra === "" || extra ) {
			num = parseFloat( val );
	...
	jQuery.cssHooks[ dimensión ] = {
		obtener: función( elem, calculado, extra ) {
			si (calculado) {
				...
				devolver rdisplayswap.test( jQuery.css( elem, "mostrar") ) &&
		...
		conjunto: función( elemento, valor, extra ) {
			var coincide,
				...
				// Solo lee estilos.position si la prueba tiene posibilidades de fallar
			...
			si ( esBorderBox && scrollboxSizeBuggy ) {
				restar -= Math.ceil(
					...
					boxModelAdjustment( elem, dimensión, "borde", falso, estilos ) -
					0.5
				);
			}
			...
			devuelve setPositiveNumber( elem, valor, restar );
		}
	};
} );
		...
		si (calculado) {
			return ( parseFloat( curCSS( elem, "marginLeft") ) ||
				elem.getBoundingClientRect().izquierda -
					swap( elem, { marginLeft: 0 }, función() {
...
Tween.prototipo = {
	...
	init: función( elem, opciones, prop, fin, facilitación, unidad ) {
		este.elem = elem;
		esto.prop = prop;
		esto.easing = flexibilización || jQuery.easing._default;
	...
	ejecutar: función(porcentaje) {
		...
		si (esta.opciones.duracion) {
			...
			este.pos = facilitado = porcentaje;
		}
	...
	_por defecto: {
		obtener: función(tween) {
			...
			si ( tween.elem.nodeType !== 1 ||
				...
				devolver tween.elem[ tween.prop ];
			}
...
jQuery.easing = {
	...
	_predeterminado: "swing"
};

jQuery.fx = Tween.prototype.init;

// Punto de extensión de compatibilidad con versiones anteriores <1.8
jQuery.fx.step = {};
...
función programar() {
	si ( en progreso ) {
		si ( documento.hidden === falso && ventana.requestAnimationFrame ) {
			...
			ventana.setTimeout(programación, jQuery.fx.intervalo);
		}
...
función createTween( valor, propiedad, animación ) {
	...
	para ( ; índice < longitud; índice++ ) {
		si ( ( interpolación = colección[ índice ].llamada( animación, propiedad, valor ) ) ) {

			//Hemos terminado con esta propiedad
			retorno interpolación;
		}
	}
}
...
función defaultPrefilter( elem, props, opts ) {
	...
	// Las animaciones de salto de cola secuestran los ganchos de efectos
	si ( !opts.queue ) {
	...
	para (prop en props) {
		...
		si ( rfxtypes.test( valor ) ) {
			...
			si ( valor === ( oculto ? "ocultar" : "mostrar" ) ) {
				...
				// Ignorar todos los demás datos que no funcionan como mostrar u ocultar
				} demás {
					continuar;
				}
			}
	...
	// Salga si esto es una operación sin efecto como .hide().hide()
	...
	si ( isBox && elem.nodeType === 1 ) {
		...
		// Identificar un tipo de visualización, prefiriendo los antiguos datos de mostrar/ocultar en lugar de la cascada CSS
		...
		si ( mostrar === "en línea" || mostrar === "bloque en línea" y restaurarPantalla != null ) {
			si ( jQuery.css( elem, "float" ) === "ninguno" ) {
				...
				estilo.display = "bloque en línea";
			}
		}
	}
		...
		si ( ganchos && "expandir" en ganchos ) {
			...
			para (índice en valor) {
				si ( !( índice en propiedades ) ) {
					...
					specialEasing[índice] = flexibilización;
				}
			}
		} demás {
			specialEasing[ nombre ] = flexibilización;
		}
	}
}
...
función Animación( elem, propiedades, opciones ) {
	resultado var,
		...
		diferido = jQuery.Deferred().always( función() {
			...
			// No coincida con elem en el selector :animated
			eliminar tick.elem;
		} ),
		tick = función() {
			si (detenido) {
			...
			var tiempoActual = fxNow || crearFxNow(),
				restante = Math.max( 0, animación.tiempoInicio + animación.duración - tiempoActual ),
	...
	para ( ; índice < longitud; índice++ ) {
		resultado = Animation.prefilters[ índice ].call( animación, elem, props, animation.opts );
		si ( resultado ) {
...
jQuery.Animation = jQuery.extend(Animación, {
	...
	preadolescentes: {
		"*": [ función(prop, valor) {
			...
			ajustarCSS(tween.elem, propiedad, rcssNum.exec(valor), interpolación);
			retorno interpolación;
		} ]
	},
...
jQuery.speed = function(velocidad, facilitación, fn) {
	var opt = velocidad && tipo de velocidad === "objeto" ? jQuery.extend( {}, velocidad ) : {
		completar: fn || !fn && suavizado ||
...
jQuery.fn.extend( {
	fadeTo: función(velocidad, a, facilitación, devolución de llamada) {

		// Mostrar cualquier elemento oculto después de establecer la opacidad a 0
	...
	animar: función(prop, velocidad, facilitación, devolución de llamada) {
		var vacío = jQuery.isEmptyObject(prop),
			...
			hacerAnimación = función() {
				...
				// Opere en una copia de la propiedad para que no se pierda la facilitación por propiedad
		...
		devuelve vacío || optall.queue === ¿falso?
			este.each(hacerAnimación):
			esta.cola( optall.cola, hacerAnimación );
	},
	detener: función(tipo, clearQueue, gotoEnd) {
		...
		devuelve esto.each( función() {
			...
			si ( sacar de la cola || !ir al final ) {
				jQuery.dequeue(este, tipo);
			}
		} );
	},
	terminar: función(tipo) {
		...
		devuelve esto.each( función() {
			...
			// Habilitar la bandera de acabado en datos privados
			...
			para ( índice = temporizadores.longitud; índice--; ) {
				si ( temporizadores[ índice ].elem === este y temporizadores[ índice ].cola === tipo ) {
					temporizadores[índice].anim.stop(verdadero);
					temporizadores.splice(índice, 1);
				}
			}

			// Busca cualquier animación en la cola antigua y termínala
			para ( índice = 0; índice < longitud; índice++ ) {
...
jQuery.fn.extend( {
	attr: función(nombre, valor) {
		devolver acceso(este, jQuery.attr, nombre, valor, argumentos.longitud > 1);
	},
...
jQuery.extend( {
	attr: función(elem, nombre, valor) {
		...
		// No obtener ni establecer atributos en los nodos de texto, comentarios y atributos
		si ( nTipo === 3 || nTipo === 8 || nTipo === 2 ) {
			devolver;
		}

		// Retorno a la propiedad cuando no se admiten atributos
	...
	removeAttr: función(elem, valor) {
		nombre de var,
			yo = 0,

			// Los nombres de atributos pueden contener caracteres de espacio que no sean HTML
...
jQuery.each( jQuery.expr.match.bool.source.match( /\w+/g ), función( _i, nombre ) {
	...
	attrHandle[ nombre ] = función( elem, nombre, isXML ) {
		var ret, manejar,
			nombre_minúsculas = nombre.toLowerCase();
...
jQuery.extend( {
	prop: función(elem, nombre, valor) {
		var ret, anzuelos,
			nType = elem.nodeType;

		// No obtener ni establecer propiedades en los nodos de texto, comentarios y atributos
		si ( nTipo === 3 || nTipo === 8 || nTipo === 2 ) {
			devolver;
		}
...
// en la opción
// El getter garantiza que se seleccione una opción predeterminada
...
jQuery.fn.extend( {
	addClass: función( valor ) {
		...
		classNames = clasesToArray( valor );

		si ( classNames.length ) {
			devuelve esto.each( función() {
				curValue = obtenerClase(este);
	...
	toggleClass: función(valor, valorDeEstado) {
		var nombresDeClase, nombreDeClase, i, self,
			tipo = tipo de valor,
		...
		si ( tipo de estado === "booleano" && esValido ) {
		...
		devuelve esto.each( función() {
			...
			// Alternar el nombre completo de la clase
				...
				si ( nombreClase ) {
					...
					dataPriv.set(this, "__className__", nombreDeClase);
				}

				// Si el elemento tiene un nombre de clase o si nos pasa `falso`,
				...
				si ( este.setAttribute ) {
					este.setAttribute( "clase",
						nombreClase || valor === ¿falso?
							"" :
							dataPriv.get(this, "__nombreClase__") || ""
					);
				}
			}
		} );
		...
		opción: {
			obtener: función( elem ) {
				...
				devuelve val != null ?
					...
					// Soporte: solo IE <=10 - 11
					// option.text lanza excepciones (trac-14686, trac-14858)
					// Eliminar y contraer espacios en blanco
					// https://html.spec.whatwg.org/#eliminar-y-collapse-espacios-blancos
...
jQuery.each( [ "radio", "casilla de verificación" ], función() {
	...
	si ( !support.checkOn ) {
		jQuery.valHooks[ this ].get = función( elem ) {
			devolver elem.getAttribute("valor") === null ? "activado": elem.valor;
		};
	}
} );
...
var rfocusMorph = /^(?:enfoqueenfoque|enfoquefueradesenfoque)$/,
...
jQuery.extend( jQuery.evento, {
	...
	disparador: función(evento, datos, elem, onlyHandlers) {
		...
		var i, cur, tmp, bubbleType, ontype, handle, especial, lastElement,
		...
		si ( elem.tipoNodo === 3 || elem.tipoNodo === 8 ) {
			devolver;
		}
		...
		// El llamador puede pasar un objeto jQuery.Event, un objeto o simplemente una cadena de tipo de evento
		...
		evento.rnamespace = evento.namespace ?
			nueva expresión regular ( "(^|\\.)" + espacios de nombres.join( "\\.(?:.*\\.|)" ) + "(\\.|$)" ) :
			nulo;
		...
		especial = jQuery.event.special[ tipo ] || {};
		si ( !onlyHandlers && special.trigger && special.trigger.apply( elem, data ) === falso ) {
			devolver;
		}

		// Determinar la ruta de propagación del evento con antelación, según la especificación de eventos del W3C (trac-9951)
		// Burbuja hacia el documento, luego a la ventana; busque una variable global ownerDocument (trac-9724)
		si ( !onlyHandlers && !special.noBubble && !isWindow( elem ) ) {
			...
			// Solo agregamos ventana si llegamos al documento (por ejemplo, no un objeto simple o un DOM separado)
			si ( tmp === ( elem.ownerDocument || documento ) ) {
				eventPath.push( tmp.defaultView || tmp.parentWindow || ventana );
			}
		}

		// Controladores de fuego en la ruta de eventos
		yo = 0;
		mientras ( ( cur = eventPath[ i++ ] ) && !event.isPropagationStopped() ) {
			...
			// Manejador de jQuery
			manejador = ( dataPriv.get(cur, "eventos") || Objeto.create(null))[ evento.type] &&
			...
			si ( manejar && manejar.aplicar && aceptarDatos( cur ) ) {
				evento.resultado = handle.apply(cur, datos);
		...
		si ( !onlyHandlers && !event.isDefaultPrevented() ) {
			...
			si ( ( !special._default ||
				...
				if ( ontype && isFunction( elem[ tipo ] ) && !isWindow( elem ) ) {
					...
					// Evitar que se vuelva a activar el mismo evento, ya que lo hemos indicado anteriormente
					jQuery.event.triggered = tipo;

					si ( evento.isPropagationStopped() ) {
						lastElement.addEventListener(tipo, detenerPropagationCallback);
					}

					elem[tipo]();

					si ( evento.isPropagationStopped() ) {
						lastElement.removeEventListener(tipo, detenerPropagationCallback);
					}
	...
	// Aprovechar un evento de donante para simular uno diferente
	// Se usa solo para eventos `focus(in | out)`
...
variedad
	corchete = /\[\]$/,
	rCRLF = /\r?\n/g,
	rsubmitterTypes = /^(?:enviar|botón|imagen|reiniciar|archivo)$/i,
...
jQuery.param = función(a, tradicional) {
	prefijo var,
		...
		agregar = función( clave, valorOFunción ) {
			...
			// Si el valor es una función, invocarla y usar su valor de retorno
	...
	si ( Array.isArray( a ) || ( a.jquery && !jQuery.isPlainObject( a ) ) ) {
		...
		// Si es tradicional, codificar de la forma "antigua" (la forma 1.3.2 o anterior
...
variedad
	...
	/* Prefiltros
	 * 1) Son útiles para introducir tipos de datos personalizados (ver ajax/jsonp.js para ver un ejemplo)
	 * 2) Estos se llaman:
	...
	// Evitar la secuencia de caracteres del prólogo de comentario (trac-10098); debe apaciguar la pelusa y evadir la compresión
...
función addToPrefiltersOrTransports(estructura) {
	...
	función de retorno (dataTypeExpression, func) {
		...
		var tipo de datos,
			yo = 0,
			tiposDeDatos = expresiónDeTipoDeDatos.toLowerCase().match(rnothtmlwhite) || [];
...
// Función de inspección base para prefiltros y transportes
función inspectPrefiltersOrTransports(estructura, opciones, originalOptions, jqXHR) {
	...
	función inspeccionar( tipoDatos ) {
		...
		jQuery.each( estructura[ tipoDatos ] || [], función( _, prefiltroOrFactory ) {
			...
			si (tipo de datosOrTransport === "cadena" &&
...
// que acepta opciones "planas" (no para extenderlas en profundidad)
...
función ajaxHandleResponses(s, jqXHR, respuestas) {
	...
	// Verificamos si estamos tratando con un tipo de contenido conocido
	si ( ct ) {
		para (escribir en el contenido) {
			si ( contenido[tipo] y contenido[tipo].prueba(ct)) {
	...
	si ( tipoDatosFinal ) {
		if (finalDataType! == tipos de datos [0]) {
...
función ajaxConvert(s, respuesta, jqXHR, isSuccess) {
	var conv2, actual, conv, temporal, anterior,
		convertidores = {},

		// Trabajar con una copia de dataTypes en caso de que necesitemos modificarla para la conversión
		tiposDeDatos = s.tiposDeDatos.slice();

	// Crear un mapa de convertidores con claves en minúsculas
	si ( tiposDeDatos[1]) {
	...
	mientras ( actual ) {
		...
		si (actual) {
			...
			// Convertir la respuesta si el tipo de datos anterior no es automático y difiere del actual
				...
				// Aplicar convertidor (si no hay equivalencia)
				si ( conv !== verdadero ) {
...
jQuery.extend( {
	...
	Configuración de ajax: {
		...
		asíncrono: verdadero,
		Tipo de contenido: "application/x-www-form-urlencoded; conjunto de caracteres=UTF-8",
	...
	ajaxSetup: función(objetivo, configuraciones) {
	...
	ajax: función(url, opciones) {
		...
		// Fuerza que las opciones sean un objeto
		opciones = opciones || {};
		...
		transporte var,
			...
			// manejador de tiempo de espera
			temporizador de tiempo de espera,

			// Variable de limpieza de URL
			urlAnchor,

			// Estado de la solicitud (se vuelve falso al enviarse y verdadero al finalizar)
			terminado,
			...
			jqXHR = {
				...
				getResponseHeader: función(tecla) {
					var coincidencia;
					si ( completado ) {
						si ( !respuestaEncabezados ) {
							encabezados de respuesta = {};
				...
				// Cadena sin formato
				obtenerTodosLosEncabezadosDeRespuesta: función() {
		...
		diferido.promesa( jqXHR );

		// Agregue el protocolo si no se proporciona (los prefiltros podrían esperarlo)
		...
		s.type = opciones.metodo || opciones.tipo || s.metodo || s.tipo;
		...
		si ( s.crossDomain == null ) {
			...
			// p. ej. http://ejemplo.com:80x/
			intentar {
				urlAnchor.href = s.url;

				// Soporte: sólo IE <=8 - 11
				// La propiedad de host de Anchor no se configura correctamente cuando s.url es relativo
		...
		// Eliminar hash para simplificar la manipulación de la URL
		...
		si ( !s.tieneContenido ) {
			...
			si ( s.cache === falso ) {
				...
				sin caché = ( rquery.test( cacheURL ) ? "&" : "?" ) + "_=" + ( nonce.guid++ ) +
					sin almacenar en caché;
			}
		...
		// Establezca el encabezado Accepts para el servidor, dependiendo del tipo de datos
		...
		si (s.antesDeEnviar&&
			...
			// Abortar si no lo hizo ya y regresar
		...
		función realizada(estado, nativeStatusText, respuestas, encabezados) {
			...
			si ( esÉxito ) {
				...
				// Si tenemos datos, convertámoslos
				} demás {
				...
				// Extrae el error de statusText y normalízalo para los no abortos
				error = textoDeEstado;
			...
			si (fireGlobals) {
				globalEventContext.trigger( esÉxito ? "ajaxÉxito" : "ajaxError",
...
jQuery._evalUrl = función(url, opciones, doc) {
	devolver jQuery.ajax( {
		...
		// El convertidor predeterminado es un poco torpe pero funciona.
		convertidores: {
...
jQuery.fn.extend( {
	...
	wrapInner: función(html) {
		si ( esFuncion( html ) ) {
			devuelve esto.each( función( i ) {
				jQuery(este).wrapInner(html.call(este, i));
			} );
		}

		devuelve esto.each( función() {
			var self = jQuery(este),
...
var xhrSuccessStatus = {
		...
		// El protocolo de archivo siempre arroja el código de estado 0, se asume 200
		0:200,
...
support.cors = !!xhrSupported && ( "conCredenciales" en xhrSupported );
soporte.ajax = xhrSupported = !!xhrSupported;

jQuery.ajaxTransport( función( opciones ) {
	var devolución de llamada, errorCallback;

	// Solo se permite entre dominios si es compatible mediante XMLHttpRequest
	si ( soporte.cors || xhrSupported && !opciones.crossDomain ) {
		devolver {
			enviar: función( encabezados, completo ) {
				...
				si ( !opciones.crossDomain && !encabezados[ "X-Solicitado-Con" ] ) {
					...
					función de retorno() {
						si ( devolución de llamada ) {
							...
							si ( tipo === "abortar" ) {
								...
								// Soporte: sólo IE <=9
								// En una interrupción nativa manual, IE9 lanza
								...
								completo(
									...
									// Para XHR2 sin texto, deje que quien llama lo gestione (gh-2498)
				...
				intentar {
					...
					xhr.send(opciones.tieneContenido && opciones.datos || null );
				} captura ( e ) {

					// trac-14683: Solo volver a lanzar si esto aún no se ha notificado como un error
					si ( devolución de llamada ) {
						lanzar e;
					}
				}
...
jQuery.ajaxPrefilter("json jsonp", función(s, originalSettings, jqXHR) {
	...
	var callbackName, sobrescrito, responseContainer,
		jsonProp = s.jsonp !== falso && ( rjsonp.test( s.url ) ?
			...
			tipo de s.data === "cadena" &&
				( s.contentType || "" )
					.indexOf( "aplicación/x-www-form-urlencoded" ) === 0 &&
	...
	si ( jsonProp || s.dataTypes[ 0 ] === "jsonp" ) {
		...
		// Obtener el nombre de devolución de llamada, recordando el valor preexistente asociado con él
		callbackName = s.jsonpCallback = isFunction( s.jsonpCallback ) ?
			s.jsonpCallback() :
			s.jsonpCallback;

		// Insertar devolución de llamada en la URL o datos del formulario
		si ( jsonProp ) {
		...
		s.converters[ "script json" ] = función() {
			si ( !contenedorDeRespuestas ) {
				jQuery.error( callbackName + " no fue llamado" );
			}
			devolver responseContainer[ 0 ];
		};
		...
		jqXHR.siempre(función() {
			...
			// Guardar de nuevo como gratuito
			si ( s[nombreDeDevoluciónDeCall]) {
				...
				oldCallbacks.push(nombreDeDevoluciónDeCall);
			}

			// Llamar si fuera una función y tuviéramos una respuesta
...
// En Safari 8, documentos creados mediante document.implementation.createHTMLDocument
...
jQuery.offset = {
	setOffset: función( elem, opciones, i ) {
		...
		// Establezca la posición primero, en caso de que la parte superior/izquierda se establezcan incluso en un elemento estático
...
jQuery.fn.extend( {
	...
	desplazamiento: función(opciones) {
		...
		si ( argumentos.longitud ) {
			opciones de retorno === ¿indefinido?
				este :
				esto.each( función( i ) {
					jQuery.offset.setOffset(este, opciones, i);
				} );
		}
	...
	posición: función() {
		...
		// Los elementos de posición fija se desplazan desde la ventana gráfica, que siempre tiene un desplazamiento cero.
		...
		devolver {
			superior: offset.top - parentOffset.top - jQuery.css( elem, "marginTop", true ),
	...
	offsetParent: función() {
		devuelve este.map( función() {
			...
			mientras ( offsetParent && jQuery.css( offsetParent, "posición" ) === "estático" ) {
				desplazamientoPadre = desplazamientoPadre.desplazamientoPadre;
			}

			retorna offsetParent ||documentElement;
		} );
	}
} );
...
jQuery.each( { scrollLeft: "pageXOffset", scrollTop: "pageYOffset" }, función( método, propiedad ) {
	...
	jQuery.fn[método] = función(val) {
		devolver acceso(este, función(elem, método, val) {
			...
			si ( val === indefinido ) {
				devolver ganar ? ganar[prop] : elem[método];
			}
...
// en lugar de hacer que el módulo css dependa del módulo offset, simplemente verifiquelo aquí
...
// Crea los métodos innerHeight, innerWidth, height, width, externalHeight y externalWidth
jQuery.each( { Altura: "altura", Ancho: "ancho" }, función( nombre, tipo ) {
	jQuery.each( {
		relleno: "interior" + nombre,
		contenido: tipo,
		"": "exterior" + nombre
	}, función( valor predeterminadoExtra, funcName ) {

		// El margen es solo para altura exterior y ancho exterior
		jQuery.fn[funcName] = función(margen, valor) {
			...
			devolver acceso(este, función(elem, tipo, valor) {
				...
				si ( elem.nodeType === 9 ) {
					...
					// O bien scroll[Ancho/Alto] o offset[Ancho/Alto] o client[Ancho/Alto],
					// el que sea mayor
					devuelve Math.max(
				...
				valor de retorno === ¿indefinido?
					...
					// Establezca el ancho o la altura del elemento
...
jQuery.fn.extend( {
	...
	delegado: función(selector, tipos, datos, fn) {
		devuelve esto.on( tipos, selector, datos, fn );
	},
	anular la delegación: función(selector, tipos, fn) {
...
jQuery.each(
	( "desenfoque enfoque enfoque externo redimensionar desplazamiento clic doble clic " +
...
// Requerir que la "ejecución de espacios en blanco" comience desde un espacio que no sea en blanco
// para evitar el comportamiento O(N^2) cuando el motor intente hacer coincidir "\s+$" en cada posición de espacio.
...
jQuery.isFunction = esFuncion;
jQuery.isWindow = esVentana;
jQuery.camelCase = camelCase;
jQuery.type = toType;

jQuery.now = Fecha.now;
...
jQuery.isNumeric = función(obj) {
	...
	var tipo = jQuery.tipo(obj);
	devolver ( tipo === "número" || tipo === "cadena") &&

		// parseFloat NaNs falsos positivos de transmisión numérica ("")
...
// derivado de los nombres de archivos, y jQuery normalmente se entrega en minúsculas
...
// Tenga en cuenta que para una portabilidad máxima, las bibliotecas que no sean jQuery deben
// se declaran como módulos anónimos y evitan establecer un valor global si un
// El cargador AMD está presente. jQuery es un caso especial. Para más información, consulte
// https://github.com/jrburke/requirejs/wiki/Actualización de bibliotecas existentes#wiki-anon
